...
public class CamelCaseIndex implements Index<String, String> {
    ...
    private void processModifyChange(final ModifyChange modifyChange) {
        processDeleteChange(
                new DeleteChange(modifyChange.timestamp,
                        new FilePointer(modifyChange.oldFileName, 0),
                        modifyChange.getOldFileContent())
        );
        processAddChange(
                new AddChange(modifyChange.timestamp,
                        new FilePointer(modifyChange.newFileName, 0),
